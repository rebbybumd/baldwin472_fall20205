...
No. Some of the data sets have columns in different orders. However, all of the column names are the same.
...
```
```{r}
maryland_population_with_voters <- read_csv('maryland_population_with_voters')
```
...
```{r}
maryland_population_with_voters|> left_join(county_voters_2022, join_by("COUNTY"))
```
...
```{r}
maryland_population_with_voters|> left_join(county_voters_2022)
```
...
It contains Zipcode, popoulation of 18 and over, and various populations by race. I also think it includes includes various housing data, such as people who cohabitate.
...
```{r}
maryland_population_with_voters_and_demographics|> left_join(maryland_demographics, by=c("COUNTY"="NAME"))
```
