...
"LING889", "LING200", "LING689", "LING499", "LING899", "LING698", "LING898", "LING896", "LING79…
...
**Answer**
There is a term number, but it's a coded term that might need to be adjusted to interpret the real term. It is also giving us a class count. For example, the first term entry is 201712 and the class count is 487. 
...
Yes. the entries are now from the highest class count to the lowest. 
...
The data set is now arranged in three columns. First by term in ascending order, then by department in alphabetical order. The third column lists the class count.
...
The three colums are now term, class counts, and total seats. It is arranged first by descending term. Then by descending total seats. 
...
It first lists the data by descending term. It is not sorting by total classes. Then it gives us the sum of the total seats for each term. Then the mean of the total seats. Then the median for the seats.  
...
Since I'm turning this in late on the weekend, and I need to ask for help, I'm going to submit it like this so I can move on to the next two exercises. 
